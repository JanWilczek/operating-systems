    ...
    if (dirinfo != NULL)
    {
        ...
        while (current_file != NULL)
        {
            ...
            if (compare_dates(time_info, &compared_date, comparison_operator))
            {
                ...
                if (S_ISDIR(current_file_status.st_mode))
                {
                    pid_t child_pid = vfork();

                    if (child_pid == 0)
                    {
                        printf("| ls usage on folder: %s  using process with pid = %d |\n", current_file->d_name, getpid());
                        execlp("ls", "ls", "-l", absolute_file_path, NULL);
                        exit(EXIT_SUCCESS);
                    }
                    else
                    {
